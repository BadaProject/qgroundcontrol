...
QGC_LOGGING_CATEGORY(BadaLog, "BadaLog")

BadaFlyViewOptions::BadaFlyViewOptions(BadaOptions* options, QObject* parent)
...
bool BadaFlyViewOptions::showMultiVehicleList(void) const
{
    return false;
}

// This custom build has it's own custom instrument panel. Don't show regular one.
bool BadaFlyViewOptions::showInstrumentPanel(void) const
{
    return false;
}

BadaOptions::BadaOptions(BadaPlugin*, QObject* parent)
    : QGCOptions(parent)
{
}

QGCFlyViewOptions* BadaOptions::flyViewOptions(void)
{
    if (!_flyViewOptions) {
        _flyViewOptions = new BadaFlyViewOptions(this, this);
    }
    return _flyViewOptions;
}

// Firmware upgrade page is only shown in Advanced Mode.
bool BadaOptions::showFirmwareUpgrade() const
{
...
bool BadaOptions::wifiReliableForCalibration(void) const
{
    return true;
}

BadaPlugin::BadaPlugin(QGCApplication *app, QGCToolbox* toolbox)
    : QGCCorePlugin(app, toolbox)
{
    _options = new BadaOptions(this, this);
    _showAdvancedUI = false;
}

BadaPlugin::~BadaPlugin()
{
}

void BadaPlugin::setToolbox(QGCToolbox* toolbox)
{
    QGCCorePlugin::setToolbox(toolbox);

    // Allows us to be notified when the user goes in/out out advanced mode
    connect(qgcApp()->toolbox()->corePlugin(), &QGCCorePlugin::showAdvancedUIChanged, this, &BadaPlugin::_advancedChanged);
}

void BadaPlugin::_advancedChanged(bool changed)
{
...
//-----------------------------------------------------------------------------
void BadaPlugin::_addSettingsEntry(const QString& title, const char* qmlFile, const char* iconFile)
{
    Q_CHECK_PTR(qmlFile);
...
QGCOptions* BadaPlugin::options()
{
    return _options;
}

QString BadaPlugin::brandImageIndoor(void) const
{
    return QStringLiteral("/custom/img/CustomAppIcon.png");
}

QString BadaPlugin::brandImageOutdoor(void) const
{
    return QStringLiteral("/custom/img/CustomAppIcon.png");
}

bool BadaPlugin::overrideSettingsGroupVisibility(QString name)
{
...
bool BadaPlugin::adjustSettingMetaData(const QString& settingsGroup, FactMetaData& metaData)
{
...
void BadaPlugin::paletteOverride(QString colorName, QGCPalette::PaletteColorInfo_t& colorInfo)
{
...
QQmlApplicationEngine* BadaPlugin::createQmlApplicationEngine(QObject* parent)
{
