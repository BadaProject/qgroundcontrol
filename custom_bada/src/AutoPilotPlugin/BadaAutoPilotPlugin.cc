...
#include "BadaAutoPilotPlugin.h"

#include "QGCApplication.h"
#include "QGCCorePlugin.h"

BadaAutoPilotPlugin::BadaAutoPilotPlugin(Vehicle* vehicle, QObject* parent)
    : PX4AutoPilotPlugin(vehicle, parent)
{
    // Whenever we go on/out of advanced mode the available list of settings pages will change
    connect(qgcApp()->toolbox()->corePlugin(), &QGCCorePlugin::showAdvancedUIChanged, this, &BadaAutoPilotPlugin::_advancedChanged);
}

// This signals that when Advanced Mode changes the list of Vehicle Settings page also changed
void BadaAutoPilotPlugin::_advancedChanged(bool)
{
    _components.clear();
...
const QVariantList& BadaAutoPilotPlugin::vehicleComponents()
{
